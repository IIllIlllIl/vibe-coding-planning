...
""",
    ...
    assert "--retry-backoff" in result.stdout
...
def test_submit_apptainer_sif_preheat_defaults_to_remote_user_from_config(
    tmp_path: Path,
) -> None:
    fake_bin = tmp_path / "bin"
    fake_bin.mkdir()
    ...
    local_root = REPO_ROOT / ".tmp_hpc_smoke" / "test_sif_preheat_remote_user"
    config = _write_preheat_config(local_root)
    config.write_text(
...
def test_submit_apptainer_sif_preheat_accepts_zero_timeout(tmp_path: Path) -> None:
    fake_bin = tmp_path / "bin"
    fake_bin.mkdir()
    fake_ulhpc = fake_bin / "ulhpc-submit"
    fake_ulhpc.write_text(
    ...
    local_root = REPO_ROOT / ".tmp_hpc_smoke" / "test_sif_preheat_zero_timeout"
    config = _write_preheat_config(local_root)
