...
def pcce_semantic_sha256(config: SWEVerifiedPCCEConfig) -> str:
    root = Path(__file__).resolve().parents[2]
    sources = sorted((root / "src" / "swe_verified_pcce").glob("*.py"))
    return _stable(
        {
            "schema": 1,
            "sources": {
...
def _case_dict(case: PCCECase, *, include_outcome: bool = True) -> dict[str, Any]:
    ...
    if not include_outcome:
        # PC workers need repository identity and issue text, never official
        # tests, gold patch, or the baseline PCE outcome.
        source.update(
            test_patch="",
            fail_to_pass=[],
            pass_to_pass=[],
    ...
    return (
        "\n".join(
            [
                ...
                (
                    f"{shlex.quote(hpc.python_bin)} -m src.swe_verified_pcce.worker "
...
class SWEVerifiedPCCEHPCExecutor:
    def __init__(self, config: SWEVerifiedPCCEConfig) -> None:
        self.config = config
    ...
    def _run_batch(
        ...
        review_index = int(extras[0].get("review_index", 0)) if extras else 0
        time_limit = (
            self.config.phase_times.ce
            if phase == "ce"
            else self.config.phase_times.first_review
            if review_index == 1
            else self.config.phase_times.revision_review
        )
        runtime = SlurmTaskBatch(replace(self.config.hpc, time=time_limit))
        ...
        for index, (case, extra) in enumerate(zip(cases, extras, strict=True)):
            ...
            payload = {
                ...
                "case": _case_dict(case, include_outcome=phase == "ce"),
                **extra,
            }
            if (
                manifest.is_file()
