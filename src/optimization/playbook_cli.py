...
def run_from_config(path: str | Path, *, agents: Any | None = None, optimize_fn=None):
    ...
    if agents is None and raw.get("execution", {}).get("backend") == "hpc_slurm":
        ...
        proposal_agents = HPCPlaybookProposalAgents(
            executor, maximum_tokens=int(raw["length"]["maximum_visible_tokens"])
        )
        ...
        adapter = PlaybookGEPAAdapter(None, proposer, batch_checker=checker)
    else:
        if agents is None:
        ...
        adapter = PlaybookGEPAAdapter(runtime.checker, proposer)
    kwargs = {}
