"""Independent Slurm transport for frozen SWE-Verified PCE regeneration."""
...
from src.swe_verified_pce.evaluator import _terminal
...
def recover_exhausted_evaluator_timeout(
    *,
    batch_dir: Path,
    task: TaskFiles,
    fingerprint: str,
    ...
    if max_attempts != 3:
        raise ValueError("evaluator Slurm-timeout recovery requires three attempts")
    ...
    def checkpoint(phase: str) -> dict[str, Any] | None:
        path = checkpoint_dir / f"{phase}.json"
        if not path.is_file():
            return None
        value = json.loads(path.read_text(encoding="utf-8"))
        if value.get("checkpoint_identity") != identity or value.get("phase") != phase:
            raise ValueError("evaluator-timeout checkpoint identity mismatch")
        payload = value.get("payload")
        if not isinstance(payload, dict):
            raise ValueError("evaluator-timeout checkpoint payload is invalid")
        return payload

    plan = checkpoint("plan")
    code = checkpoint("code")
    if plan is None or code is None or checkpoint("evaluate") is not None:
        return None
    marker_path = checkpoint_dir / "evaluate_started.json"
    if not marker_path.is_file():
        return None
    marker = json.loads(marker_path.read_text(encoding="utf-8"))
    if (
        marker.get("checkpoint_identity") != identity
        or marker.get("phase") != "evaluate"
    ):
        raise ValueError("evaluator-start marker identity mismatch")
    attempts = []
    for attempt in range(1, max_attempts + 1):
        status_path = task.attempts_dir / f"attempt_{attempt:02d}" / "slurm_status.json"
        if not status_path.is_file():
            return None
        status = json.loads(status_path.read_text(encoding="utf-8"))
        if (
            status.get("state") != "TIMEOUT"
            or status.get("instance_id") != task.instance_id
            or status.get("task_index") != task.index
        ):
            return None
        attempts.append(status)
    evaluator_result = _terminal(
        outcome="unknown",
        reason="slurm_evaluator_timeout_after_attempts",
        evaluator_resolved=None,
        evidence={
            "evaluator_attempts_exhausted": max_attempts,
            "slurm_attempts": attempts,
        },
    )
    atomic_json(
        checkpoint_dir / "evaluate.json",
        {
            "schema_version": 1,
